...
if ($email) {
    $userData = fetchUserDataByEmail($email); // Function to fetch user profile data
    $fullName = ($userData['first_name'] ?? '') . ' ' . ($userData['last_name'] ?? '');
} else {
    header('Location: ../view/login.html'); // Redirect to login if no session
    exit;
}
?>
...
<body>
    <div class="container">
        <h1>User Profile</h1>
        <form method="POST" action="../controller/updateProfile.php"> <!-- New file for profile update -->
